...
Given an array of n integers where n > 1, return an array output such
 that output[i] is equal to the sum of all the elements of nums except nums[i]. 
...
It is guaranteed that the sum of the elements of any prefix or 
suffix of the array (including the whole array) fits in a 32 bit integer.
 */
import java.util.*;
public class arr13{

    public static void WithoutMe(int n , int arr[]) {
        int sum =0;
        for(int i=0;i<n;i++){
            for(int j=0;j<n;j++){
                if(arr[i]==arr[j]){
